  ...
  return (
    <div>
      ...
      <div style={{ background: `linear-gradient(135deg, ${colors.deep}, ${colors.mid})`, color: '#fff', padding: isMobile ? '40px 20px' : '52px 24px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: fonts.heading, fontWeight: 800, fontSize: isMobile ? '28px' : '36px', marginBottom: '10px' }}>Returns &amp; Refunds</h1>
        <p style={{ fontFamily: fonts.body, fontSize: '14px', opacity: 0.85, maxWidth: '500px', margin: '0 auto' }}>
          We want you to be 100% happy with your purchase. Our 30-day return policy makes it easy.
        </p>
      </div>

      <div style={{ maxWidth: '900px', margin: '0 auto', padding: isMobile ? '28px 16px 48px' : '40px 24px 64px' }}>
        {/* Steps */}
        <h2 style={{ fontFamily: fonts.heading, fontWeight: 800, fontSize: '20px', color: colors.deep, marginBottom: '20px' }}>How to Return an Item</h2>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: '12px', marginBottom: '32px' }}>
          {STEPS.map(s => (
            <div key={s.n} style={{ background: 'white', borderRadius: '16px', padding: isMobile ? '16px 14px' : '20px', boxShadow: shadow.card, position: 'relative' }}>
              <div style={{ width: '36px', height: '36px', background: `linear-gradient(135deg, ${colors.bright}, ${colors.mid})`, borderRadius: radius.md, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '10px' }}>
                <Icon name={s.icon} size={18} color="#fff" />
              </div>
              <div style={{ position: 'absolute', top: '14px', right: '14px', fontFamily: fonts.heading, fontWeight: 800, fontSize: '24px', color: colors.light, lineHeight: 1 }}>{s.n}</div>
              <div style={{ fontFamily: fonts.heading, fontWeight: 700, fontSize: '13px', color: colors.deep, marginBottom: '6px' }}>{s.title}</div>
              <p style={{ fontFamily: fonts.body, fontSize: '11px', color: colors.muted, lineHeight: 1.6 }}>{s.desc}</p>
            </div>
          ))}
        </div>
        ...
        {/* Timeline */}
        <div style={{ background: colors.light, borderRadius: '16px', padding: '20px', border: `1.5px solid ${colors.mint}`, marginBottom: '20px' }}>
          <h3 style={{ fontFamily: fonts.heading, fontWeight: 800, fontSize: '15px', color: colors.deep, marginBottom: '14px' }}>Refund Timeline</h3>
          {TIMELINE.map((t, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < TIMELINE.length - 1 ? `1px solid ${colors.successBg}` : 'none', gap: '10px', flexWrap: 'wrap' }}>
              <span style={{ fontFamily: fonts.body, fontSize: '13px', color: colors.textSec, fontWeight: 500 }}>{t.label}</span>
              <span style={{ fontFamily: fonts.heading, fontSize: '12px', fontWeight: 700, color: colors.deep, flexShrink: 0 }}>{t.time}</span>
            </div>
          ))}
        </div>

        {/* CTA */}
        <div style={{ background: `linear-gradient(135deg, ${colors.deep}, ${colors.mid})`, borderRadius: '16px', padding: isMobile ? '24px 20px' : '28px 32px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: '16px' }}>
          <div>
          ...
          <button
            ...
            style={{ background: '#fff', color: colors.deep, border: 'none', borderRadius: radius.lg, padding: '12px 24px', fontFamily: fonts.heading, fontWeight: 800, fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}
          >
